<template>
  <PageComponent>
    ...
    <div v-else-if="draftStore.drafts.length || forms.data.length">
      <div class="relative card bg-base-100 shadow-xl p-4">
          ...
          <div v-for="draft in draftStore.drafts" :key="draft.id">
            <div class="flex flex-col py-4 px-6 bg-slate-50 hover:bg-slate-200 rounded-md">
              ...
              <div class="flex justify-end items-center mt-3 gap-3">
                <router-link :to="{ name: 'FormsModule', query: { draftId: draft.id } }" class="btn btn-sm btn-ghost">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-4">
                    <path
      ...
      <div class="relative card bg-base-100 shadow-xl mt-3 p-4">
        <div>
          <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 md:grid-cols-3">
