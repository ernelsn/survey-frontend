<template>
  ...
  <form class="mt-8 space-y-6" @submit.prevent="authenticate.login(form)">
    <input type="hidden" name="remember" value="true" />
    <div class="rounded-md shadow-sm -space-y-px">
      <div>
        <label for="email-address" class="sr-only">Email address</label>
        <input id="email-address" name="email" type="email" autocomplete="email" v-model="form.email" required=""
      ...
      <div>
        ...
        <input id="password" name="password" type="password" autocomplete="password" required="" v-model="form.password"
    ...
    <div>
      <button type="submit" :disabled="authenticate.loading"
        ...
        <span v-if="authenticate.loading" class="loading loading-dots loading-sm"></span>
        Sign in
      </button>
    </div>
  </form>
