  ...
  <PageComponent title="Dashboard">
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 text-gray-700">
      ...
      <div class="bg-white shadow-md p-3 text-center flex flex-col order-2 lg:order-4 animate-fade-in-down"
        ...
        <h3 class="text-2xl font-semibold">Total Response</h3>
        <div class="text-8xl pb-4 font-semibold flex-1 flex items-center justify-center">
          {{ data.totalFormResponse }}
        </div>
      </div>
      <div class="row-span-2 animate-fade-in-down order-3 lg:order-1 bg-white shadow-md p-4">
        ...
        <div v-if="data.latestForm">
          ...
          <div class="flex justify-between text-sm mb-1">
            <div>Published:</div>
            <div>{{ data.latestForm.is_published ? "Active" : "Draft" }}</div>
          </div>
          <div class="flex justify-between text-sm mb-1">
            <div>Total questions:</div>
            <div>{{ data.latestForm.total_questions }}</div>
          </div>
          <div class="flex justify-between text-sm mb-3">
            <div>Total response:</div>
            <div>{{ data.latestForm.responses }}</div>
          </div>
          <div class="flex justify-between">
            <router-link :to="{ name: 'FormsModule', params: { id: data.latestForm.id } }" class="btn btn-ghost">
            ...
            <router-link :to="{ name: 'FormResponses', params: { id: data.latestForm.id } }" class="btn btn-ghost">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-4">
      ...
      <div class="bg-white shadow-md p-3 row-span-2 order-4 lg:order-3 animate-fade-in-down"
        ...
        <div class="flex justify-between items-center mb-3 px-2">
          <h3 class="text-2xl font-semibold">Latest response</h3>
        ...
        <div v-if="data.latestFormResponses && data.latestFormResponses.length">
          <a href="#" v-for="response in data.latestFormResponses" :key="response.id"
            class="block p-2 hover:bg-gray-100/90">
            <div class="font-semibold">{{ response.form_title }}</div>
            <small>
              Response made at:
              <i class="font-semibold">{{ response.end_date }}</i>
            </small>
            <div>
              Score: {{ response.correct_answers }} / {{ response.total_questions }}
            </div>
          </a>
        </div>
        <div v-else class="text-gray-600 text-center py-16">
